...
func getTopRatedMovieMockData() -> MovieResult? {
    var response: MovieResult?
    do {
        guard let data = topRatedMovieMockResponse else {
            print("Nil data in tvShow Video Mock data")
            return nil
        }
        response = try JSONDecoder().decode(MovieResult.self, from: data)
    } catch let error {
...
var topRatedMovieMockData: MovieResult? = getTopRatedMovieMockData()
